...
const C = {
    add: (z1: Complex, z2: Complex): Complex => ({ re: z1.re + z2.re, im: z1.im + z2.im }),
    sub: (z1: Complex, z2: Complex): Complex => ({ re: z1.re - z2.re, im: z1.im - z2.im }),
    mul: (z1: Complex, z2: Complex): Complex => ({ re: z1.re * z2.re - z1.im * z2.im, im: z1.re * z2.im + z1.im * z2.re }),
    ...
    cbrt: (z: Complex): Complex => {
        const r = Math.sqrt(z.re * z.re + z.im * z.im);
        const theta = Math.atan2(z.im, z.re);
        return { re: Math.cbrt(r) * Math.cos(theta / 3), im: Math.cbrt(r) * Math.sin(theta / 3) };
    }
}
